...
namespace GitHub.TreeSitter
{
    ...
    internal struct LoadPath
    {
#if WINDOWS

        internal const string DLL_PATH = @".\bin\win32\tree-sitter.dll";
        //internal const string DLL_PATH = @"D:\Code\Programs\CodeReader\Test\bin\Release\net9.0\bin\win32\tree-sitter.dll";

#elif MACOS
        internal const string DLL_PATH = @".\bin\mac\tree-sitter.dylib";
#elif LINUX
        internal const string DLL_PATH = @".\bin\linux\tree-sitter.so";
#endif
    }
    ...
    public sealed class TSParser : IDisposable
    {
        ...
        {
            var ptr = ts_parser_parse_string_encoding(
                Ptr,
                oldTree != null ? oldTree.Ptr : IntPtr.Zero,
                input,
                (uint)input.Length * 2,
        ...
        */
        ...
        [DllImport(LoadPath.DLL_PATH, CallingConvention = CallingConvention.Cdecl)]
    ...
    public struct TSNode
    {
        ...
        public void clear()
        { id = IntPtr.Zero; tree = IntPtr.Zero; }

        public bool is_zero()
        { return (id == IntPtr.Zero && tree == IntPtr.Zero); }

        public string type()
        { return Marshal.PtrToStringAnsi(ts_node_type(this)); }
        ...
        { return ts_node_start_byte(this) / sizeof(ushort); }

        public TSPoint start_point()
        { var pt = ts_node_start_point(this); return new TSPoint(pt.row, pt.column / sizeof(ushort)); }

        public uint end_offset()
        { return ts_node_end_byte(this) / sizeof(ushort); }

        public TSPoint end_point()
        { var pt = ts_node_end_point(this); return new TSPoint(pt.row, pt.column / sizeof(ushort)); }

        public string to_string()
        { var dat = ts_node_string(this); var str = Marshal.PtrToStringAnsi(dat); ts_node_string_free(dat); return str; }

        public bool is_null()
        { return ts_node_is_null(this); }
        ...
        {
            if (obj != null && obj is TSNode other)
            {
    ...
    public sealed class TSCursor : IDisposable
    {
        ...
        { return lang.fields[current_field_id()]; }
        ...
        { return new TSCursor(ts_tree_cursor_copy(ref cursor), lang); }
    ...
    public sealed class TSLanguage : IDisposable
    {
        ...
        { return (symbol != UInt16.MaxValue) ? symbols[symbol] : "ERROR"; }
